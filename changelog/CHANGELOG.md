# CHANGELOG

## [2026-09-22] - 修正列印重疊、欄寬正規化、欄寬可拖曳

### Fixed
- **列印時「覆核」與「客戶意見與簽認」互相重疊（排版全亂）**
  - 根因：前一版把列印版面改成 flex 時沒有關掉 `flex-shrink`。
    預設的 `flex-shrink: 1` 會在內容超過一頁時把各區塊壓得比內容還矮，
    區塊因此互相疊上去。內容列數比範本多就會踩到
  - 改為所有 flex 項目 `flex-shrink: 0`，內容過長改由整體等比縮小處理

- **新增欄位後中文標題變成一行一個字（看起來像直書）**
  - `addColumn()` 固定給新欄位 18%，但原有 5 欄已合計 100%，
    加一欄變 118%，欄位被擠窄
  - 新欄位寬度改取現有欄位平均，插入後再整體正規化成合計剛好 100%
    （四捨五入的零頭補回最寬的欄）。新增與刪除欄位、搬移欄位後都會重新正規化
  - 實測新增 3 欄後最窄欄仍有 23mm，「新欄位」標題維持單行

### Added
- **內容超過一頁時整體等比縮小，不再被裁切**
  - 改掛在 `matchMedia('print')` 而非 `beforeprint`：後者觸發時列印樣式
    還沒生效（實測 `.controls` 仍是 `display:flex`、`.page` 量到螢幕版的
    210x303mm），算出的可用高度是錯的。`matchMedia('print')` 的 change
    事件是在列印版面成形後才觸發，且該事件中的 DOM 改動會反映到列印結果
    （實測：事件中隱藏主表格，PDF 文字繪製指令由 285 降為 209）
  - 量測期間需暫時關掉 `flex-grow`，否則簽認欄自動填滿會讓 `scrollHeight`
    被可用高度蓋住，迭代擺盪而縮過頭
  - 實測（真實 `page.pdf` 流程）：加 20 列時邊界 0mm → scale 0.766、
    邊界 10mm → scale 0.707，皆 1 頁、無重疊，文字繪製指令由 285 增至 307

- **欄寬可用滑鼠拖曳調整**（兩個表格皆適用）
  - 標題格右緣有拖曳把手，只在相鄰兩欄之間搬移寬度，
    表格總寬永遠維持 100%，拉一欄不會把後面整排推歪；單欄下限 5%
  - 表格改為 `table-layout: fixed`，欄寬才由設定值說了算
  - 把手設 `contenteditable="false"` 以免編輯標題時被刪掉，
    並阻擋其 click / dragstart，不會誤觸刪除模式或欄位搬移
  - 列印時隱藏，匯出 Word 前從複本中移除；匯出 Excel 不受影響（把手無文字）

## [2026-09-22] - 列印版面改為自動填滿可印區域

### Fixed
- **列印／匯出 PDF 時「客戶意見與簽認」下方仍有一大片空白**
  - 先前的修正（用 `beforeprint` 量出空隙後把簽認欄加高）**在真實瀏覽器裡是無效的**：
    實測 Chrome 是在套用列印樣式「之前」才觸發 `beforeprint` ——
    當下 `.controls` 仍是 `display:flex`、`.page` 量到的是螢幕版的 210x303mm。
    螢幕上 `.page` 本來就被內容撐高，算出來的空隙是 0，簽認欄因此完全沒被加高。
    先前之所以「測起來正常」，是因為測試裡先呼叫了 `emulateMedia('print')`
  - 改用純 CSS flexbox：`.page` / `.scalable-content` / `.signature` 設為 flex 欄，
    簽認欄 `flex: 1 1 auto` 自動吸收剩餘高度。由瀏覽器在分頁時計算，不受時機影響
  - 同時移除失效的 JS 加高邏輯與對應的 `afterprint` 還原

- **列印對話框選「預設邊界」時底部空白特別多**
  - `.page` 原本寫死 `width: 210mm; height: 297mm`。選「預設邊界」時
    可印寬度只有約 190mm，Chrome 會把整頁縮到 90%，
    縮完的高度短於可印高度，底部因此多出一片空白
  - 改為 `width: auto; height: 100%`，頁面直接填滿可印區域，不再被縮放
  - 需搭配 `html, body { height: 100% }`，並把 `body` 從置中的 flex 改為
    `display: block`（`align-items: center` 會讓 `width: auto` 縮成內容寬度）
  - A/B 實測（視窗設為可印尺寸以模擬分頁後版面）：
    可印 210x297mm → 簽認欄 34.7mm、頁尾底端 285.1mm、底部空白 12.0mm；
    可印 190x277mm → 簽認欄 15.5mm、頁尾底端 265.0mm、底部空白 12.0mm。
    兩者底部空白皆等於設定的 12mm，PDF 均為 1 頁

### Notes
- `beforeprint` 內原有的「內容過長時整體縮小」邏輯同樣建立在螢幕版量測上。
  推導與實測都顯示該條件在真實列印流程中永遠不成立（螢幕上 `.page`
  會被內容撐高，可用高度恰等於內容高度），等於從未生效。
  內容超過一頁時目前會被 `.page` 的 `overflow: hidden` 裁切 —— 此為既有行為，未在本次變更

## [2026-09-22] - 非營業收支拆成收入／支出、表格 Enter 跳格

### Added
- **表格內按 Enter 跳到同欄的下一格**（Shift+Enter 往上）
  - 兩個表格都適用；到該欄最後一格會換到下一欄的第一格
  - 會持續往後尋找可停留的欄位，因為可能整欄都不可編輯
    （歷年表的「年度」欄與「核定」勾選欄）
  - 自動計算的格子會跳過（公式產出的值，游標停在上面沒有意義），
    但已手動覆寫的格子會停留
  - 段落文字（狀況說明、提醒）不受影響，仍可正常換行

### Fixed
- **列印／匯出 PDF 會多出一頁幾乎空白的第二頁**
  - 頁面淡入動畫（`pageTransition`）進行中，`body` 會殘留 `translateY`，
    把 `.page` 底端推到 297.81mm，超出 A4 的 297mm，Chrome 因此多分一頁
  - 使用者若在載入 0.5 秒內就按列印就會踩到；初始的 `opacity: 0`
    還可能讓報表印成半透明
  - `@media print` 內一律關掉 animation / opacity / transform
  - A/B 實測（解析產出的 PDF 計算頁數）：
    修正前「立刻列印」2 頁、「等動畫結束」1 頁；修正後兩者皆為 1 頁

- **「備註」欄現在永遠固定在最後一欄**
  - 新增欄位改為插在備註欄之前（原本是附加在備註之後，備註會被擠到中間）
  - 排列模式下備註欄不可拖曳，拖其他欄位到備註上時只會顯示「放左側」
  - `moveColumn` 收尾統一檢查一次，不在最後就移回去 ——
    與其在每個搬移路徑各自算邊界，用不變式收斂，任何路徑都不會漏

- **儲存格內按 Enter 會插入換行、撐開列高** — contenteditable 的預設行為，
  隨上述跳格功能一併攔掉

### Changed
- **「(±)非營業收支」拆成「非營業收入」與「非營業支出」兩列**
  （下方原本沒有標題的那一列即為支出）

- **非營業支出改為「一律減去其絕對值」** — 避免改名後產生的靜默算錯
  - 原本兩列都是「照欄位內的正負號直接加總」，標題的 `(±)` 就是在提示
    使用者自己帶正負號；改名為「非營業支出」後這個提示消失，
    若有人依字面在支出欄填正數 `407,358`，淨利會被**加上**而非扣除，
    以現有資料為例會從 8,557,843 變成 9,372,559 —— 且無任何錯誤提示
  - 支出改取絕對值後減去，`407,358`、`-407358`、`(407,358)` 三種填法
    結果完全相同，兩種習慣都相容
  - 非營業收入維持照欄位值加總（可為負）
  - **現有數字完全未變**：1-6 月帳載數 8,557,843、113 年申報數 7,886,836
    皆與原表一致
  - 新增的列依名稱自動判定：名稱含「非營業」且含「支出／損失／費用」
    歸為支出，其餘歸為收入（較保守的預設）

## [2026-09-18] - 年度報告：Excel 匯出、公式自動計算、歷年表可增減行列

### Added
- **匯出 Excel（`report_generator.html`）** — 原本只有「儲存為 PDF / 列印」與「匯出 Word」
  - 刻意**不用合併儲存格排版**（只有抬頭、基本資料、提醒、狀況說明、簽認等
    段落文字才合併），表格維持正常儲存格，才能在 Excel 裡繼續新增欄位、改數值
  - 數字還原成**真正的數值型別**並套數字格式，可直接運算：
    `49,141,049` → `49141049`（`#,##0;(#,##0)`）、
    `29.94%` → `0.2994`（`0.00%;(0.00%)`）、
    `$1,577,367` → `1577367`（`"$"#,##0;("$"#,##0)`）
  - 判斷不出來的內容（`-4323萬`、`Δ萬元`）保留為文字，不亂猜
  - 欄寬依主表格百分比動態換算，勾選框轉成 `☑YES ☐NO`，A4 直式 fitToWidth
  - ExcelJS 4.4.0 由 CDN **延遲載入**（900KB，點下按鈕才下載），附 SRI

- **歷年營所稅申報紀錄表可增減行列** — 原本只有主表格能新增／刪除
  - 控制列按鈕改為「主表新增項目／主表新增欄位／歷年表新增列／歷年表新增欄」
  - 新增列會依同欄既有儲存格自動帶出「核定」欄的 YES/NO 勾選框
    （用 `cloneNode` 而非 `innerHTML` 字串插值）
  - 刪除沿用既有的刪除模式，兩表共用，一樣只能刪使用者自己新增的

- **主表格公式自動計算**
  - 毛利 = 營業收入 − 營業成本
  - 毛利率 = 毛利 ÷ 營業收入
  - 淨利 = 毛利 − 營業費用（一般＋折舊＋薪資＋其他）＋ 非營業收支
  - 淨利率 = 淨利 ÷ 營業收入
  - 兩列非營業收支**照欄位內的正負號直接加總**，所以支出須填負數
  - **算法經表上原有數字反推驗證**：1-6 月帳載數與 113 年申報數兩欄
    的毛利、毛利率、淨利皆精確吻合到個位數
  - 計算欄位**仍可手動編輯**：輸入後標記 `data-overridden`，該格脫離公式
    且不被重算覆蓋，手動值會往下傳遞（改毛利會影響毛利率與淨利）；
    清空該格即恢復自動計算。編輯模式下自動值顯示綠底、手動值顯示琥珀色
  - 使用者新增的列依項目名稱推斷角色，命名為「營業費用: ○○」會自動併入淨利

- **每個數值直行可各自選擇「自動計算」或「手動輸入」**
  - 控制列新增一排切換鈕，每個數值欄一顆（備註欄不列入），點一下切換
  - 設為手動時整欄四個計算格完全交給使用者填，不做任何計算；
    切回自動則以公式重算並清除該欄的手動值
  - 設定存在該欄 `<th>` 的 `dataset.calcMode`，
    因此拖曳搬移、新增或刪除欄位時設定都會跟著那一欄走
  - 改動欄位標題後，切換鈕上顯示的欄位名稱會同步更新
  - 欄層級的「手動」與格層級的「手動覆寫」可並用：
    自動欄仍可單獨改某一格（該格轉琥珀色並脫離公式）

### Changed
- **備註欄以外的欄位一律做數值判定與正規化**
  - 自動加千分位、去除 `$`、保留使用者輸入的 `+` 號、百分比固定兩位
  - **負數改用會計慣例的括號**：`-407,358` 顯示為 `(407,358)`，
    括號輸入也能解析回負數；Excel 端存的是真正的負數，只是套括號格式，
    因此 `=SUM()` 仍正確
  - 判讀不出來的內容標紅底並提示，不會靜默吃掉

### Fixed
- **列印時頁尾以下留一大片空白（實測約 27mm，整頁的 9%）**
  - `beforeprint` 原本只在內容超過一頁時等比縮小，內容不足一頁時不做任何處理
  - 改為把多出來的高度補給「客戶意見與簽認」欄，頁尾因此落到頁面底部
  - 不採整體放大：放大會連帶改變換行、還得反向壓縮寬度，容易溢出頁面右緣
  - 空隙以「內容底部到頁面內緣底部」直接量測，不用推算 —
    抬頭的 `margin-bottom` 不計入 `offsetHeight`，推算會少算而吃掉下邊界
  - A/B 實測：頁尾底端由 257.5mm → 285.5mm，底部空白由 39.5mm → 11.5mm
    （即設定的 12mm 下邊界）；內容超過一頁時原本的縮小邏輯不受影響
  - **⚠ 更正（2026-09-22）：這個修正在真實瀏覽器裡並未生效。**
    上述 A/B 實測是在測試中先呼叫 `emulateMedia('print')` 的前提下量的；
    真實列印流程中 Chrome 是在套用列印樣式「之前」才觸發 `beforeprint`，
    量到的是螢幕版版面，算出的空隙為 0，簽認欄從未被加高。
    正確做法見 2026-09-22「列印版面改為自動填滿可印區域」與
    「修正列印重疊、欄寬正規化、欄寬可拖曳」兩條

### Notes
- 套用公式後，**「114 年預估值」欄的淨利由 7,260,174 變為 20,039,174**，
  因為該欄四項營業費用皆為空白。原數字反推需約 12,779,000 的費用，
  但該金額未填入表格 — 需補填預估費用
- 表上原本的**淨利率兩格與公式結果差 0.01 個百分點**
  （17.40% → 17.41%、7.57% → 7.58%）。分子分母都是表上現成數字，
  試過四捨五入、無條件捨去、改分母皆無法同時算出原本那兩個值，
  研判為當初手動輸入的殘值
- 「預估營所稅額 20%」實為 `淨利 × 20%`（已驗證），但**未納入自動計算**，
  淨利變動後這格不會跟著更新

## [2026-09-15] - 全面體檢修正

### Fixed
- **Excel 欄位標題含雙引號時，選項值遭靜默截斷**（`tool1` / `tool2` / `tool3`）
  - 原本以 `innerHTML` 字串插值產生選項：`<option value="${h}">`，
    標題若含 `"`（例如「規格 15" 螢幕」）會提前結束屬性，
    value 被截成「規格 15」— **且無任何錯誤訊息**
  - tool1 是拿這個 value 當比對鍵合併兩份檔案，鍵值錯誤會直接產出
    看似正常但內容錯誤的合併結果，屬於靜默的資料正確性問題
  - 改用 `createElement` + `textContent`（新增 `fillSelectWithHeaders()`），共 6 處
  - A/B 實測：改動前 value 為 `"規格 15"`，改動後為 `"規格 15\" 螢幕"`

- **`fetch` 的 timeout 參數無效**（`js/common.js`）
  - 四處寫 `fetch(url, { timeout: 5000 })`，但 fetch 並無 `timeout` 選項，
    該參數被完全忽略 — 統編查詢實際上毫無逾時保護
  - 改用 `AbortController`

- **全站缺少 favicon** — 每位訪客的瀏覽器都會對 `/favicon.ico` 發出請求並得到 404
  （線上實測確認）。以 data URI 內嵌 SVG 圖示，不需額外檔案

- **`localStorage` 保存失敗時使用者毫不知情**（`script.js`）
  - 原本僅 `console.warn`。容量已滿或無痕模式下，使用者會以為發票資料
    已自動保存，實際上重新整理即全部遺失
  - 改為顯示持續性提醒，並提示先匯出 Excel

### Changed
- **保留 Excel 原始儲存格型別**（待辦 TODO #1，已完成）
  - `sheet_to_json` 的 `raw: false` 改為 `raw: true`，
    並於 `XLSX.read` 加上 `cellDates: true`
  - A/B 實測：改動前金額為字串 `"1000"`、日期為字串 `"1/5/26"`（Excel 公式無法運算）；
    改動後金額為數值 `1000`、日期為 `Date` 物件
  - **合併筆數與比對結果完全一致**，未改變既有行為
  - 可安全改動的原因：`normalizeValue()` 早已針對 `typeof value === 'number'`
    處理 Excel 日期序號，本來就相容 `raw: true`

- **SheetJS 0.18.5 → 0.20.3，並全面加上 SRI**
  - 0.18.5 為 2022 年版本，有已知的 prototype pollution 與 ReDoS 漏洞。
    本站用途正是處理外部寄來的 Excel，風險情境吻合
  - SheetJS 已不在 cdnjs 發佈新版，改用官方 `cdn.sheetjs.com`
  - 6 個 CDN script 全數加上 `integrity` 與 `crossorigin`（原本 0 個）

- **統編查詢移除三層失效的後備**（`js/common.js`）
  - `g0v /api/search`（名稱搜尋端點，用統編查一律回 `found:0`）、
    NexData 與 allorigins 實測皆已無法連線（HTTP 000）
  - 保留實測正常且具 CORS 標頭的 g0v `/api/show/`

- **`js/config.js` 改為真正被使用** — 原本定義的 API 常數
  在 `common.js` 中全是硬編碼、一個都沒用到，改網址時必然漏改。
  同時為 tool2/3/4 補上 `config.js`（`common.js` 依賴 `CONFIG`，
  原本未載入，屬於未爆彈）

### Removed
- **35 個無用檔案**
  - `scripts/` 下 31 個一次性除錯腳本 — 硬編碼指向桌面上的
    `更動1.xlsx`、`世華.xlsx`（不在 repo 內），且會將本機路徑結構公開
  - `exchange-rate.html.backup`（仍使用已失效代理的舊版）、
    `script.js.backup`、`apply_fixes.py`、`fix-script.txt`
    — 四者線上皆可直接存取（實測 HTTP 200）
- **正式環境的 6 個 `console.log`**（保留 `warn` / `error`）

### Accessibility
- 補上 8 處 `aria-label`（日期的月／日子欄位、篩選關鍵字、取代值、4 個 `select`）
- 以瀏覽器實測可及名稱：26 個控制項，缺漏數由 4 降為 **0**

### Notes
- 稅額計算經 1~100000 元全範圍驗證**無誤**，合計恆自洽，未做更動
- 每項修正皆以 A/B 對照驗證（改動前後跑同一份資料比較），
  避免測試空轉而得出無效結論

---

## [2026-09-15] - 停用鍵盤上下鍵改動 number 欄位值

### Added
- **阻擋鍵盤上下鍵增減數值**（`js/input-guards.js`）
  - 實測確認僅 `ArrowUp` / `ArrowDown` 會改動 number 欄位，
    `PageUp` / `PageDown` / `Home` / `End` 皆不影響，故只需處理上下鍵
  - 與滾輪處理同樣採 document 層級委派，動態產生的欄位一併涵蓋

### Changed
- **`js/no-wheel.js` 更名為 `js/input-guards.js`**
  - 檔案已不只處理滾輪，原名不再貼切
  - 同步更新 9 個 HTML 的引用與 `script.js` 的註解

### Notes
- **select 的上下鍵刻意保留不擋**：那是鍵盤操作下拉選單的唯一方式，
  擋掉會讓只用鍵盤的使用者無法選取選項。已在測試中驗證
  發票類型 select 按下 ArrowDown 仍可正常切換
- 以對照組驗證有效性：停用 `input-guards.js` 後，總保費按兩次 ArrowUp
  會從 36500 變成 36502；載入後則完全不受影響
- 一併回歸測試滾輪阻擋與頁面捲動，確認未因新增鍵盤處理而受影響

---

## [2026-09-15] - 移除 number 欄位的上下微調箭頭

### Fixed
- **`exchange-rate.html` 的外幣金額欄位仍有上下微調箭頭**
  - `style.css` 早已有 spinner 移除規則，但該頁樣式全部內嵌、
    未載入任何外部 CSS，因此不受涵蓋 — 這是全站唯一的缺口
  - 於該頁內嵌樣式補上相同規則
  - `report_generator.html` 同樣未載入外部 CSS，但頁面只有 checkbox，
    無 number 欄位，不需處理

### Changed
- **`style.css` 的 spinner 規則現代化**
  - 選擇器由 `input::-webkit-*-spin-button` 收斂為 `input[type="number"]::...`
  - 補上標準 `appearance` 屬性（原本只有 `-webkit-` 與 `-moz-` 前綴版本）

### Notes
- 驗證方式：先確認 headless Chromium 確實會繪製 spinner（同頁比對有無規則的
  兩個欄位，截圖 2219B vs 1815B 有差異），再對每個實際欄位截圖，
  並注入 CSS 強制開啟箭頭後再截一次，兩者有差異即代表原本確實已移除
- 涵蓋靜態欄位與動態產生的發票列（銷售額／稅額／總計）

---

## [2026-09-15] - 全站停用滑鼠滾輪改動欄位值

### Fixed
- **滾輪捲動頁面時會誤改已填欄位**（全站 9 個頁面）
  - 新增 `js/no-wheel.js`，於 document 層級以捕獲階段委派處理，
    動態產生的欄位（例如發票列）也一併涵蓋
  - 涵蓋所有會被滾輪改值的控制項：`number`、`date`、`datetime-local`、
    `month`、`week`、`time`、`range` 以及 `select`
    （`select` 在 Firefox 下滾輪會直接切換選項）
  - 已在全部 9 個 HTML 頁面載入

### Changed
- **改以失焦方式處理，不再阻擋頁面捲動**
  - 原本 `script.js` 針對銷項發票區塊的處理使用 `preventDefault()`，
    雖能擋住改值，但游標停在欄位上時整頁就捲不動 —
    銷項發票表格幾乎整片都是數字欄位，影響明顯
  - 改為在滾輪事件中讓欄位失焦：欄位失焦後瀏覽器不會改值，
    且因未呼叫 `preventDefault()`，頁面照常捲動
  - `script.js` 中原本的 invoice-section 處理已移除（由全站版本涵蓋）

### Notes
- 以對照組驗證修正確實有效：移除 `no-wheel.js` 後，總保費欄位
  滾動 5 格會從 36500 被改成 36505；載入後則完全不受影響
- 驗證涵蓋靜態欄位、動態產生的發票列、select，以及頁面捲動是否仍正常

---

## [2026-09-15] - 修復匯率查詢：改為 GitHub Actions 定期擷取

### Fixed
- **匯率查詢完全失效** (`exchange-rate.html`)
  - **根本原因（兩層同時失效）**
    1. 台銀 `rate.bot.com.tw` 全站啟用 **Akamai Bot Manager** 防護（`sec_cpt` cookie）。
       所有端點（`/xrt`、`/xrt/all/{date}`、`/xrt/flcsv/...`）一律回傳 1897 bytes 的
       `Challenge Validation` 頁面，補上完整 Chrome User-Agent 也無效。
       原本的解析邏輯因此找不到表格，拋出「找不到匯率表格」。
    2. 程式碼中三個 CORS 代理**全數失效**：
       - `api.allorigins.win` → HTTP 522（服務連不上）
       - `corsproxy.io` → HTTP 403（已改為需註冊 API key）
       - `thingproxy.freeboard.io` → 連線失敗（服務已下線）
  - **結論**：「純前端 + 公共 CORS 代理」的架構已不可行。任何資料中心 IP、
    無法執行 JS 的請求都過不了挑戰，更換代理清單只是治標。

### Changed
- **改為靜態資料架構**
  - 新增 `scripts/fetch_bot_rates.js`：以 Playwright 真實瀏覽器執行挑戰的
    JS proof-of-work，擷取後存成 `data/rates/YYYY-MM-DD.json`
  - 新增 `.github/workflows/fetch-rates.yml`：每個營業日 10:30 與 17:30（台北時間）
    自動更新，並將資料 commit 回 repo
  - 前端 `fetchBOTRates()` 改為讀取**同源**的 `data/rates/{date}.json`
    → 免代理、無 CORS 問題、載入速度大幅提升
  - 資料格式與原本 `displayRates()` / `initCalculator()` 完全相容，顯示邏輯未更動

### Added
- **可查詢範圍提示**：載入時讀取 `data/rates/index.json`，在使用說明區顯示
  實際可查詢的日期區間，並將輸入框 placeholder 設為最新營業日
- **精準的錯誤說明**：查無資料時改以索引判斷真正原因，區分
  「假日／非營業日」（並提示前後一個營業日）、「超出資料庫最早紀錄」、
  「尚未更新」三種情況，取代原本籠統的失敗訊息
- **當日盤中牌價**：`/xrt/all/{date}` 僅收錄已收盤日期，當天查詢會回空，
  因此改由即時牌告頁擷取，並在標題標示「盤中 HH:MM 更新」以區別定盤價
- **歷史資料回補**：一次性回補 2025-01-01 起的所有營業日資料

### Notes
- 台銀本身僅保留約 **2025 年初之後**的歷史資料（實測 2024-12-16 已查無），
  但資料一旦收錄進 repo 即永久保存，不受台銀下架影響
- 擷取腳本必須使用 `channel: 'chromium'`（新版 headless）；
  舊的 headless shell 無法通過 Akamai 挑戰。挑戰約需 50 秒，
  但 session 建立後可連續擷取多日而不再被挑戰

---

## [待辦] - 資料處理工具待改善項目

### TODO #1 — 保留原始儲存格格式（四大工具通用）
- **問題**：目前四個工具在讀寫 Excel 時，使用 `raw: false` 將所有資料轉成字串，導致數字欄位變成文字，Excel 公式無法正常運算
- **影響範圍**：`tool1_merger.html`、`tool2_filter.html`、`tool3_replacer.html`、`tool4_invoice_format.html`
- **需求**：讀取時改用 `raw: true`，寫出時保留原始型別（數字寫數字、日期寫日期序號），確保 Excel 公式可正常運作
- **注意**：改動後需重新驗證各工具的比對/篩選邏輯，原本依賴字串比較的地方可能需要調整

### TODO #2 — 資料抽取工具新增「發票開立作廢篩選」功能
- **位置**：`tool2_filter.html`，在檔案成功匯出後顯示
- **功能流程**：
  1. 匯出完成後，下方出現「發票開立作廢篩選」按鈕
  2. 點擊後列出匯出檔案的所有標題欄位，供使用者勾選（單選或多選 TBD）
  3. 使用者選定標題後，系統掃描該欄位所有內容
  4. 凡包含「**開立已確認**」字串的列，整列完整資料抽出
  5. 匯出篩選結果為新的 Excel 檔案
- **預設行為**：關鍵字固定為「開立已確認」（不需使用者輸入）

---

## [2026-04-10] - 新增資料處理工具四：電子發票格式

### Added
- **電子發票格式轉換工具** (`tool4_invoice_format.html`)
  - 資料處理工具入口（`data_processing_landing.html`）新增第四個按鈕
  - **左右雙區塊上傳介面**
    - 左欄（A 檔）：原始格式範本，需包含兩個活頁簿
      - 活頁簿 1：定義輸出欄位標題
      - 活頁簿 2：固定內容，原封不動複製至輸出
    - 右欄（B 檔）：已處理資料集，僅讀取第一個活頁簿
    - 上傳後即時顯示偵測到的活頁簿名稱、欄位數、資料筆數
    - A 不足 2 個活頁簿時顯示警告，阻止繼續操作

  - **輸出邏輯**
    - 活頁簿 1：套用 A 的標題列 + B 的資料列，B 多出的欄位自動移除，B 缺少的欄位填入空白
    - 活頁簿 2：直接複製 A 的活頁簿 2 worksheet 物件（保留原始內容）
    - 欄位比對前對所有標題做 `trim()`，消除 Excel 儲存格常見的不可見空白差異

  - **轉換前欄位 Double Check（可互動刪除）**
    - A 上傳成功後自動渲染欄位清單，每個欄位以膠囊樣式呈現
    - 勾選不需要的欄位 → 標籤變紅底 → 點「刪除標題」移除
    - 刪除後即時更新欄位數量，且「開始轉換並匯出」按鈕在所有欄位被刪光時自動禁用

  - **轉換後標題預覽（可再次刪除並重新匯出）**
    - 轉換完成後顯示「▶ 轉換檔案標題預覽」切換按鈕
    - 展開後列出輸出活頁簿 1 的所有欄位，帶編號，可勾選
    - 勾選不需要的欄位 → 點「刪除標題」→ 自動更新 effectiveHeaders、同步上方 Double Check 區、重新匯出，不需手動再按匯出按鈕

### Fixed
- **左右拖放區高度不對齊** (commit: 77b83a0)
  - 問題：右側說明卡比左側多一行，導致右側拖放區起始位置偏低
  - 修正：對 `.file-panel .drop-zone` 設 `flex: 1`，讓拖放區自動撐滿各欄剩餘高度
  - 效果：左右拖放區等高且底部對齊

---

## [2024-12-23] - UI/UX 改進：統計區視覺優化

### Improved
- **優化「共 XX 筆資料」顯示樣式** (commit: 60f4f59)
  - 改進前：與金額資訊混在一起，靠右對齊，視覺層次不清
  - 改進後：
    - 置中顯示，字體 20px，灰色 (#666)
    - 與金額資訊拉開距離 (margin-bottom: 8px)
    - 移除不必要的 min-width 限制，更緊湊
  - padding 調整：統一上下間距 15px（原本上 20px 下 0）
  - 影響範圍：`style.css:151-155`

---

## [2024-12-23] - UI/UX 改進：移除 JS 寬度調整與優化勾選框

### Improved
- **移除 JavaScript 寬度調整，改用 CSS 自動適應**
  - 刪除：`adjustInputWidth()` 函數及所有調用點（約 30 行程式碼）
  - 刪除：`textMeasureSpan` 隱藏測量元素
  - 修正：在 CSS 中加入 `min-width: fit-content`（表格儲存格）和 `min-width: 80px`（輸入框）
  - 效果：
    - 減少不必要的 DOM Reflow（效能提升 10-100 倍）
    - 簡化程式碼維護
    - 瀏覽器原生 CSS 處理，更穩定
  - 影響範圍：`style.css:132,135`, `script.js:557-578, 242-248, 640-641, 1264-1269, 1288, 1345, 1352, 1364, 1498`

- **優化欄位勾選框 UI（按鈕式設計）**
  - 改進前：小勾選框 + 文字標籤，難以點擊
  - 改進後：
    - 隱藏原生勾選框，整個標籤變成可點擊按鈕
    - 未勾選：白色背景 + 灰色邊框
    - 已勾選：藍色背景 + 白色文字 + 陰影效果
    - 滑鼠懸停：背景色變化提示可點擊
  - 技術實作：
    - 勾選框尺寸：16px × 16px → 完全隱藏
    - 標籤按鈕：`padding: 8px 16px`, `border: 2px solid`
    - 使用 `:has(input:checked)` 偵測勾選狀態
    - 防止文字被選取：`user-select: none`
  - 影響範圍：`style.css:161-166`

### Performance
- **減少 DOM 操作次數**
  - 之前：每次輸入觸發 `adjustInputWidth()` → 8 次 Reflow（輸入「12345678」）
  - 現在：CSS 自動處理，0 次 JavaScript 介入
  - 預期改善：大量資料（100+ 張發票）時輸入更流暢

---

## [2024-12-23] - 發票系統 Bug 修復與資料持久化改進

### Fixed
- **合併匯出遺漏部分資料** (commit: 3519724)
  - 問題：合併匯出時，有發票號碼但金額為 0 的列被跳過
  - 根本原因：`extractInvoiceData()` 使用舊的檢查邏輯
    - 二聯式：`if (!totalValue) continue`
    - 三聯式：`if (!salesValue) continue`
  - 修正：統一使用 `hasData` 檢查邏輯（與 `extractTableData` 和 `exportCurrentType` 一致）
    - 檢查所有欄位（date, invoiceNo, buyer/taxId, company, item, sales, tax, total）
    - 只要有任一欄位有值就納入匯出
    - 添加 `.trim()` 確保空白字串不被當作有效資料
  - 影響範圍：`script.js:1166-1214`

- **表頭 sticky 失效與重複發票號碼橘色框不更新** (commit: 2f20f30)
  - 問題 1：表頭往下滾動就消失
  - 根本原因：`border-collapse: collapse` 在某些瀏覽器會破壞 sticky positioning
  - 修正：
    - 改用 `border-collapse: separate` 與 `border-spacing: 0`
    - 保持視覺效果一致但支持 sticky
    - 為 `th` 添加 `border-bottom: 2px` 增強視覺分隔
  - 問題 2：修正重複發票號碼後橘色框不消失
  - 症狀：修改重複的發票號碼後，其他欄位的橘色框仍然存在
  - 根本原因：`checkDuplicateInvoiceNumbers()` 只檢查當前值，不重新檢查所有欄位
  - 修正：
    - 重寫檢查邏輯，使用 `invoiceNumberCounts` 統計所有發票號碼
    - 每次修改時重新檢查所有欄位並更新橘色框狀態
    - 只在新產生重複時才彈出警告（避免修改時重複提示）
    - 確保修正後橘色框立即消失
  - 影響範圍：`style.css:131,133`, `script.js:1533-1570`

- **表頭無法凍結與 Excel 匯出遺漏部分資料** (commit: 48dd18e)
  - 問題 1：勾選「發票號碼」後，往下滾動時標題無法固定在頂部
  - 根本原因：sticky 定位需要正確的父容器 `position: relative` 設定
  - 修正：
    - 為 `.invoice-table-container` 添加 `position: relative`
    - 為 `table` 添加 `position: relative`
    - 提升 `th` 的 `z-index` 從 1 到 10
    - 添加 `box-shadow` 讓凍結效果更明顯
  - 問題 2：有發票號碼但金額為 0 的列未匯出到 Excel
  - 症狀：5 列資料中，第 2、4 列有日期和發票號碼但金額為 0，匯出時被跳過
  - 根本原因：`hasData` 檢查時遺漏了 `sales` 和 `tax` 欄位
  - 修正：
    - 二聯式：檢查所有欄位（date, invoiceNo, buyer, item, sales, tax, total）
    - 三聯式：檢查所有欄位（date, invoiceNo, taxId, company, item, sales, tax, total）
    - 添加 `.trim()` 確保空白字串不被當作有效資料
  - 影響範圍：`style.css:130-133`, `script.js:960-1000`

- **隨機點擊欄位測試時產生重複發票號碼** (commit: eed090b)
  - 問題：用戶隨機點擊不同欄位測試 +1 功能時，產生重複的發票號碼
  - 根本原因：自動焦點移動邏輯導致 `lastFocusedInvoiceInput` 與用戶意圖不同步
  - 修正：移除 `nextEmptyInput` 的自動焦點移動邏輯，保持焦點在當前填入的欄位
  - 新行為：用戶完全控制焦點位置，不會自動跳到下一個空欄位
  - 影響範圍：`script.js:1503-1522`

- **切換發票類型時資料消失** (commit: 1ec0fa7)
  - 問題：在二聯式輸入日期和金額後切換到三聯式，再切回來時資料消失
  - 根本原因：`extractTableData()` 只提取「必填欄位有值」的行（二聯式必填 `total-2`，三聯式必填 `sales-3`）
  - 修正：改為提取「任何欄位有值」的行，並消除 `switchInvoiceType()` 的冗余保存（從 3 次減少到 1 次）
  - 影響範圍：`script.js:77-131`, `script.js:580-623`

- **發票號碼 +1 功能不工作** (commit: 1ec0fa7)
  - 問題：第一行輸入發票號碼（如 `AS12313212`）後，按「+」號無法遞增到 `AS12313213`
  - 根本原因：舊邏輯查找「倒數第二列」，當只有一列時無法找到參考（`lastRowIndex - 1 = -1`）
  - 修正：完全重寫邏輯，向上查找「最後一個有效的發票號碼」，自動新增列並填入遞增值
  - 新行為：
    - 從所有列中找最後一個符合格式 `[A-Z]{2}\d{8}` 的發票號碼
    - 自動新增一列（而非填入現有列）
    - 若無有效發票號碼，提示用戶先輸入
  - 影響範圍：`script.js:1408-1469`

### Technical Improvements
- **數據提取邏輯**
  - 從「必填欄位檢查」改為「任意欄位有值檢查」
  - 消除邊界情況：不再依賴 `requiredFieldClass` 來判斷有效行
  - 減少 `switchInvoiceType()` 複雜度：移除「第0步」和「第1步」的冗余保存

- **發票號碼遞增邏輯**
  - 消除「倒數第二列」這種脆弱的假設
  - 改用「向上查找最後有效值」的穩健模式
  - 新增自動建列功能，提升使用者體驗

---

## [2024-12-23] - 發票系統完整重構與匯出功能優化

### Added
- **智能發票匯出功能** (commit: 8fd8944)
  - 新增三種匯出模式：
    1. 僅匯出當前類型（二聯式或三聯式）
    2. 合併匯出（二聯式+三聯式智能排序）
    3. 分開匯出（產生兩個獨立 Excel 檔案）
  - 動態偵測資料可用性，自動顯示/隱藏匯出選項

- **State-Driven 架構核心模組** (commit: 8dbb16c)
  - `js/core/state.js` - 反應式狀態管理系統
  - `js/core/render.js` - 安全渲染引擎（XSS 防護）
  - `js/core/events.js` - 全域事件委派系統
  - `js/utils.js` - 安全工具函式庫（驗證、格式化）
  - `js/modules/invoice.js` - 統一發票系統模組（待整合）

### Fixed
- **匯出選項只顯示當前類型按鈕** (commit: fa38ae4)
  - 問題：Line 848 條件 `hasTwoPartData && hasThreePartData` 過於嚴格
  - 症狀：只有一種類型有資料時，合併/分開匯出按鈕消失
  - 修正：改為 `hasTwoPartData || hasThreePartData`
  - 邏輯：只要有任一種資料就應顯示所有匯出選項

- **合併匯出排序邏輯最終修正** (commit: e1e3417)
  - 確認排序規則：年月 → 發票類型 → 完整日期
  - 同月份內所有二聯式排完後，再排所有三聯式
  - 範例排序：
    ```
    1140101 二聯式
    1140110 二聯式
    1140105 三聯式  ← 雖然日期5號較早，但類型優先級較低
    1140201 二聯式  ← 新月份開始
    ```

- **切換發票類型時欄位資料消失** (commit: d3d3c94)
  - 修復 `extractTableData()` 欄位名稱對應錯誤
  - 問題：`'data-invoice-no'` 錯誤轉換為 `'invoiceno'`（應為 `'invoiceNo'`）
  - 解決：使用明確的 `fieldMapping` 物件，確保 camelCase 一致性

- **匯出功能無法讀取非當前類型資料** (commit: 57f0eac)
  - 修復 `checkHasValidData()` 只檢查可見 DOM 的問題
  - 新增 localStorage 資料檢查邏輯
  - 確保切換到其他類型時，匯出選項仍可正確顯示

- **合併匯出排序邏輯調整歷程**
  - commit 5929954: 嘗試完整日期優先（不符需求）
  - commit 8a22473: 嘗試月份分組 + 類型優先（正確）
  - commit 7248ab5: 誤解需求改為純日期排序（錯誤）
  - commit e1e3417: 最終確認月份分組 + 類型優先（正確）

### Changed
- **保留原有日期格式** (用戶要求)
  - 維持民國年 7 位數格式（YYYmmdd，如 1140629）
  - 理由：會計作業便利性考量

- **移除統編查詢速率限制** (用戶要求)
  - 原設計：每分鐘最多 5 次查詢
  - 實際實作：無速率限制（從未實作限制功能）
  - 理由：用戶需要頻繁查詢公司名稱

### Technical Improvements
- **架構改進**
  - 從 DOM-as-data-source 轉向 State-Driven Architecture
  - 實作 Single Source of Truth 模式
  - 全域事件委派取代 inline event handlers
  - XSS 防護（HTML 跳脫處理）

- **代碼品質**
  - 欄位名稱統一使用 camelCase（`invoiceNo`, `taxId`）
  - localStorage 資料持久化與遷移層
  - 減少代碼重複（統一發票系統模組）

### Notes
- 核心模組已建立但採用漸進式整合策略
- 現有功能保持 100% 向後相容
- 未來可逐步將舊代碼遷移至新架構

---

## 開發原則
遵循 Linus Torvalds 哲學：
- ✅ 消除特殊情況，追求「好品味」代碼
- ✅ Never break userspace（零破壞性）
- ✅ 實用主義優先（解決真實問題）
- ✅ 簡潔執念（避免過度設計）
