...
### Fixed
- **滾輪捲動頁面時會誤改已填欄位**（全站 9 個頁面）
  - 新增 `js/no-wheel.js`，於 document 層級以捕獲階段委派處理，
    動態產生的欄位（例如發票列）也一併涵蓋
  - 涵蓋所有會被滾輪改值的控制項：`number`、`date`、`datetime-local`、
    `month`、`week`、`time`、`range` 以及 `select`
    （`select` 在 Firefox 下滾輪會直接切換選項）
  - 已在全部 9 個 HTML 頁面載入

### Changed
- **改以失焦方式處理，不再阻擋頁面捲動**
  - 原本 `script.js` 針對銷項發票區塊的處理使用 `preventDefault()`，
    雖能擋住改值，但游標停在欄位上時整頁就捲不動 —
    銷項發票表格幾乎整片都是數字欄位，影響明顯
  - 改為在滾輪事件中讓欄位失焦：欄位失焦後瀏覽器不會改值，
    且因未呼叫 `preventDefault()`，頁面照常捲動
  - `script.js` 中原本的 invoice-section 處理已移除（由全站版本涵蓋）

### Notes
- 以對照組驗證修正確實有效：移除 `no-wheel.js` 後，總保費欄位
  滾動 5 格會從 36500 被改成 36505；載入後則完全不受影響
- 驗證涵蓋靜態欄位、動態產生的發票列、select，以及頁面捲動是否仍正常
...
- **問題**：目前四個工具在讀寫 Excel 時，使用 `raw: false` 將所有資料轉成字串，導致數字欄位變成文字，Excel 公式無法正常運算
- **影響範圍**：`tool1_merger.html`、`tool2_filter.html`、`tool3_replacer.html`、`tool4_invoice_format.html`
- **需求**：讀取時改用 `raw: true`，寫出時保留原始型別（數字寫數字、日期寫日期序號），確保 Excel 公式可正常運作
- **注意**：改動後需重新驗證各工具的比對/篩選邏輯，原本依賴字串比較的地方可能需要調整

### TODO #2 — 資料抽取工具新增「發票開立作廢篩選」功能
- **位置**：`tool2_filter.html`，在檔案成功匯出後顯示
- **功能流程**：
  1. 匯出完成後，下方出現「發票開立作廢篩選」按鈕
  2. 點擊後列出匯出檔案的所有標題欄位，供使用者勾選（單選或多選 TBD）
  3. 使用者選定標題後，系統掃描該欄位所有內容
  4. 凡包含「**開立已確認**」字串的列，整列完整資料抽出
  5. 匯出篩選結果為新的 Excel 檔案
- **預設行為**：關鍵字固定為「開立已確認」（不需使用者輸入）

---

## [2026-04-10] - 新增資料處理工具四：電子發票格式

### Added
- **電子發票格式轉換工具** (`tool4_invoice_format.html`)
  - 資料處理工具入口（`data_processing_landing.html`）新增第四個按鈕
  - **左右雙區塊上傳介面**
    - 左欄（A 檔）：原始格式範本，需包含兩個活頁簿
      - 活頁簿 1：定義輸出欄位標題
      - 活頁簿 2：固定內容，原封不動複製至輸出
    - 右欄（B 檔）：已處理資料集，僅讀取第一個活頁簿
    - 上傳後即時顯示偵測到的活頁簿名稱、欄位數、資料筆數
    - A 不足 2 個活頁簿時顯示警告，阻止繼續操作

  - **輸出邏輯**
    - 活頁簿 1：套用 A 的標題列 + B 的資料列，B 多出的欄位自動移除，B 缺少的欄位填入空白
    - 活頁簿 2：直接複製 A 的活頁簿 2 worksheet 物件（保留原始內容）
    - 欄位比對前對所有標題做 `trim()`，消除 Excel 儲存格常見的不可見空白差異

  - **轉換前欄位 Double Check（可互動刪除）**
    - A 上傳成功後自動渲染欄位清單，每個欄位以膠囊樣式呈現
    - 勾選不需要的欄位 → 標籤變紅底 → 點「刪除標題」移除
    - 刪除後即時更新欄位數量，且「開始轉換並匯出」按鈕在所有欄位被刪光時自動禁用

  - **轉換後標題預覽（可再次刪除並重新匯出）**
    - 轉換完成後顯示「▶ 轉換檔案標題預覽」切換按鈕
    - 展開後列出輸出活頁簿 1 的所有欄位，帶編號，可勾選
    - 勾選不需要的欄位 → 點「刪除標題」→ 自動更新 effectiveHeaders、同步上方 Double Check 區、重新匯出，不需手動再按匯出按鈕

### Fixed
- **左右拖放區高度不對齊** (commit: 77b83a0)
  - 問題：右側說明卡比左側多一行，導致右側拖放區起始位置偏低
  - 修正：對 `.file-panel .drop-zone` 設 `flex: 1`，讓拖放區自動撐滿各欄剩餘高度
  - 效果：左右拖放區等高且底部對齊

---

## [2024-12-23] - UI/UX 改進：統計區視覺優化

### Improved
- **優化「共 XX 筆資料」顯示樣式** (commit: 60f4f59)
  - 改進前：與金額資訊混在一起，靠右對齊，視覺層次不清
  - 改進後：
    - 置中顯示，字體 20px，灰色 (#666)
    - 與金額資訊拉開距離 (margin-bottom: 8px)
    - 移除不必要的 min-width 限制，更緊湊
  - padding 調整：統一上下間距 15px（原本上 20px 下 0）
  - 影響範圍：`style.css:151-155`
